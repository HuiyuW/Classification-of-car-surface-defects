...
class ConfusionMatrix(object):
    ...
    def summary(self):#计算指标函数
        ...
        for i in range(self.num_classes):#精确度、召回率、特异度的计算
            ...
            table_array.append([self.labels[i], Precision, Recall, Specificity,F1])
        # print(table)
        return str(acc),table, table_array

    def plot(self):#绘制混淆矩阵
        matrix = self.matrix
        acc,table, table_array = self.summary()
        # print(matrix)
        fig = plt.figure(random.randint(7,20))
        ...
        plt.xticks(range(self.num_classes), ['Dent','Other','Rim','Scratch'], rotation=45)
        # 设置y轴坐标label
        #self.labels
        plt.yticks(range(self.num_classes), ['Dent','Other','Rim','Scratch'])
        # 显示colorbar
        plt.colorbar()
        ...
        pic_name = 'static/images/AL_' +'accuracy' + '.png'
