...
def main_train(path_dataset,num,select,label_list,img_path_list):
    ...
    train_df = pd.read_csv('./results/AL_15_accuracy_0.8174_images_375select_3.csv') # read train csv from pretrain model
    ...
    limit = 50 # use new dataset to train model and get new performance
    since = time.time()
    print("-"*10)
...
##################################################################################Test
    ...
    if test_epoch_acc > 0.8174:
        print("After learning new imgs the accuracy of our model improved from 0.8174 to {}.".format(test_epoch_acc))
    else:
        print("After learning new imgs the accuracy of our model changed from 0.8174 to {}.".format(test_epoch_acc))

    confusion.plot()
    print(combine_train_df.shape[0],'images were used')
    csv_saved_name = './results/new_train_AL_' + str(num) + '_' +'select'+ '_' +str(select) + 'accuracy' + '_' + str(test_epoch_acc)+ '_' + '.csv'
    combine_train_df.to_csv(csv_saved_name,index=False,header=True)
    model_parameter_saved_name = './results/new_train_AL_' + str(num) + '_' +'select'+ '_' +str(select)+ 'accuracy' + '_' + str(test_epoch_acc)+ '_parameter.pkl'
...
if __name__ == '__main__':
    ...
    label_list = [0,1,2,0,3] #Suppose you have obtained labels for five images manually annotated by the user
